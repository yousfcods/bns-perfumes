...
// Target 4 Men's Fragrances for this fixed bundle
const MENS_OFFER_PERFUME_IDS = [
  'dior-sauvage',
  'creed-av-absolu',
  'versace-eros-energy',
  'lv-pacific-chill',
...
export default function MensOfferModal({
  isOpen,
  onClose,
  onAddBundleToCart,
}: MensOfferModalProps) {
  ...
  const offerPerfumes = useMemo(() => {
    return MENS_OFFER_PERFUME_IDS.map((id) => {
      const found = LUXURY_PERFUMES.find((p) => p.id === id);
      if (found) return found;

      // Exact uploaded product images from catalog
      const fallbackImages: Record<string, string> = {
        'dior-sauvage': 'https://res.cloudinary.com/qmmcvx8e/image/upload/v1784493425/Dior_sauvage_srf6qo.jpg',
        'creed-av-absolu': 'https://res.cloudinary.com/qmmcvx8e/image/upload/v1784493425/Creed_avantus_absolu_vi0ac2.avif',
        'versace-eros-energy': 'https://res.cloudinary.com/qmmcvx8e/image/upload/v1784493418/Versace_erose_najim_bk3wjv.webp',
        'lv-pacific-chill': 'https://res.cloudinary.com/qmmcvx8e/image/upload/v1784493424/Louis_vuitton_pacific_chill_jq4efm.webp',
      };

      return {
        id,
        brand: id.includes('dior')
          ? 'Dior'
          : id.includes('creed')
          ? 'Creed'
          : id.includes('versace')
        ...
        rating: 4.9,
        reviewsCount: 250,
        image: fallbackImages[id] || 'https://res.cloudinary.com/qmmcvx8e/image/upload/v1784493425/Dior_sauvage_srf6qo.jpg',
        sizes: ['30ml'],
        defaultSize: '30ml',
        gender: 'Men' as const,
        notes: ['Fresh', 'Woody', 'Spicy'],
        availability: 'Best Seller' as const,
        description: 'Luxury Men Fragrance X2 Concentration',
        ...
        >
          ...
          <div className="relative p-6 sm:p-8 text-center border-b border-[#C8A96A]/20 bg-gradient-to-b from-[#181510] via-[#0D0D0D] to-[#0B0B0B]">
            ...
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-serif text-white tracking-wide font-light mb-2">
              {language === 'ar'
                ? 'باقة العطور الرجالية الفاخرة X2'
                : "MEN'S X2 COLLECTION BUNDLE"}
            </h2>

            <p className="text-gray-300 font-sans text-xs sm:text-sm max-w-xl mx-auto leading-relaxed">
              {language === 'ar'
                ? 'مجموعة متكاملة تضم العطور الرجالية الأربعة الأكثر مبيعاً في باقة واحدة فاخرة (سعة 30 مل لكل عطر بتركيز مضاعف X2).'
                : 'A pre-made luxury bundle featuring all 4 iconic men’s fragrances (30ML each in double X2 concentration).'}
            </p>
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-6 mt-4 pt-4 border-t border-[#C8A96A]/15 text-xs text-[#C8A96A]">
              <span className="flex items-center gap-1.5 bg-black/60 px-3 py-1 rounded-md border border-[#C8A96A]/30">
                <Flame size={14} />
                {language === 'ar' ? '4 عطور كاملة في الباقة' : '4 Complete Fragrances'}
              </span>
              <span className="flex items-center gap-1.5 bg-black/60 px-3 py-1 rounded-md border border-[#C8A96A]/30">
                <ShieldCheck size={14} />
